...
| 状态 | 条件 | 视觉效果 |
|------|------|----------|
| `active`（活跃） | `is_online=1` 且 `last_active_at` 在 5 分钟内 | 绿色边框 + 呼吸动画 |
| `online`（在线） | `is_online=1` 且 `last_active_at` 超过 5 分钟 | 浅绿色边框，无呼吸 |
| `offline`（离线） | `is_online=0` | 白色边框，无呼吸 |

### 心跳机制

- 每个用户登录后，每 30 秒向数据库更新自己的 `last_active_at`
- 120 分钟无操作自动设为离线，清除登录状态并跳转登录页
- `last_active_at` 字段需在 PocketBase `users` 集合（`_pb_users_auth_`）的 schema 中手动添加（type: date, required: false）

### 跨用户状态同步机制

- **所有用户**的节点都应该正确显示呼吸效果，不仅仅是当前登录用户
- 当用户 A 和用户 B 同时在线时，A 的页面应该能看到 B 的呼吸效果（如果 B 处于活跃状态），反之亦然
- 状态刷新机制：每 **30 秒**从数据库查询所有在线用户（`filter: 'status = 1 && is_online = 1'`），基于最新的 `last_active_at` 重新计算每个用户的状态
- 本地缓存上次查询结果（`lastOnlineUserMap`），只有 `last_active_at` 或状态真正变化时才触发 Vue 响应式更新，减少无效 DOM 渲染
- 当前用户的状态使用本地 `lastActiveTime` 计算（更准确，避免网络延迟）
- Realtime 订阅作为辅助机制，在数据库记录变更时立即更新节点